...
class TestMain:
    ...
    @patch('workers.SemanticDiffsStorage')
    @patch('workers.SubmissionsStorage')
    @patch('workers.LLamaCPP')
    @patch('workers.MongoClient')
    @patch('workers.os.getenv')
    def test_reads_mongodb_url_from_environment(self, mock_getenv, mock_mongo_client, mock_llamacpp,
                                               mock_submissions_storage, mock_semantic_storage, mock_worker_class):
        """Reads MONGODB_URL from environment (default: mongodb://localhost:8765/)."""
        mock_getenv.side_effect = lambda key, default=None: {
            "MONGODB_URL": "mongodb://test:27017/",
            "LLAMACPP_URL": "http://test:8989",
            "TOKEN": "test-token"
        }.get(key, default)
    ...
    @patch('workers.MongoClient')
    @patch('workers.os.getenv')
    def test_reads_mongodb_url_default(self, mock_getenv, mock_mongo_client, mock_llamacpp,
                                       mock_submissions_storage, mock_semantic_storage, mock_worker_class):
        """Reads MONGODB_URL default: mongodb://localhost:8765/."""
        mock_getenv.side_effect = lambda key, default=None: default

        main()

        mock_mongo_client.assert_called_once_with("mongodb://localhost:8765/")
    ...
    @patch('workers.MongoClient')
    @patch('workers.os.getenv')
    def test_reads_llamacpp_url_from_environment(self, mock_getenv, mock_mongo_client, mock_llamacpp,
                                                  mock_submissions_storage, mock_semantic_storage, mock_worker_class):
        """Reads LLAMACPP_URL from environment (default: http://localhost:8989)."""
        mock_getenv.side_effect = lambda key, default=None: {
            "MONGODB_URL": "mongodb://test:27017/",
            "LLAMACPP_URL": "http://test:8989",
            "TOKEN": "test-token"
        ...
        assert call_kwargs["host"] == "http://test:8989"
    ...
    @patch('workers.MongoClient')
    @patch('workers.os.getenv')
    def test_reads_llamacpp_url_default(self, mock_getenv, mock_mongo_client, mock_llamacpp,
                                        mock_submissions_storage, mock_semantic_storage, mock_worker_class):
        """Reads LLAMACPP_URL default: http://localhost:8989."""
        mock_getenv.side_effect = lambda key, default=None: default
        ...
        mock_llamacpp.assert_called_once()
        call_kwargs = mock_llamacpp.call_args[1]
        assert call_kwargs["host"] == "http://localhost:8989"
    ...
    @patch('workers.MongoClient')
    @patch('workers.os.getenv')
    def test_reads_token_from_environment(self, mock_getenv, mock_mongo_client, mock_llamacpp,
                                          mock_submissions_storage, mock_semantic_storage, mock_worker_class):
    ...
    @patch('workers.MongoClient')
    @patch('workers.os.getenv')
    def test_creates_mongodb_client(self, mock_getenv, mock_mongo_client, mock_llamacpp,
                                    mock_submissions_storage, mock_semantic_storage, mock_worker_class):
    ...
    @patch('workers.MongoClient')
    @patch('workers.os.getenv')
    def test_gets_rss_database(self, mock_getenv, mock_mongo_client, mock_llamacpp,
                               mock_submissions_storage, mock_semantic_storage, mock_worker_class):
        """Gets 'rss' database."""
        mock_getenv.side_effect = lambda key, default=None: default
        mock_client_instance = MagicMock()
        ...
        mock_client_instance.__getitem__.assert_called_with("rss")
    ...
    @patch('workers.MongoClient')
    @patch('workers.os.getenv')
    def test_creates_llamacpp_instance(self, mock_getenv, mock_mongo_client, mock_llamacpp,
                                       mock_submissions_storage, mock_semantic_storage, mock_worker_class):
        """Creates LLamaCPP instance."""
        mock_getenv.side_effect = lambda key, default=None: default
    ...
    @patch('workers.MongoClient')
    @patch('workers.os.getenv')
    def test_calls_prepare_on_submissions_storage(self, mock_getenv, mock_mongo_client, mock_llamacpp,
                                                  mock_submissions_storage, mock_semantic_storage, mock_worker_class):
        """Calls prepare() on SubmissionsStorage."""
        mock_getenv.side_effect = lambda key, default=None: default
        mock_submissions_instance = MagicMock()
        mock_submissions_storage.return_value = mock_submissions_instance

        main()

        mock_submissions_instance.prepare.assert_called_once()
    ...
    @patch('workers.MongoClient')
    @patch('workers.os.getenv')
    def test_calls_prepare_on_semantic_diffs_storage(self, mock_getenv, mock_mongo_client, mock_llamacpp,
                                                      mock_submissions_storage, mock_semantic_storage, mock_worker_class):
        """Calls prepare() on SemanticDiffsStorage."""
        mock_getenv.side_effect = lambda key, default=None: default
        mock_semantic_instance = MagicMock()
        mock_semantic_storage.return_value = mock_semantic_instance

        main()

        mock_semantic_instance.prepare.assert_called_once()
    ...
    @patch('workers.MongoClient')
    @patch('workers.os.getenv')
    def test_creates_worker_instance(self, mock_getenv, mock_mongo_client, mock_llamacpp,
                                     mock_submissions_storage, mock_semantic_storage,
                                     mock_cache_store, mock_worker_class):
        """Creates Worker instance with db, llm, and cache_store."""
        mock_getenv.side_effect = lambda key, default=None: default
        mock_client_instance = MagicMock()
        mock_mongo_client.return_value = mock_client_instance
        mock_db = MagicMock()
        ...
        mock_cache_store.return_value = mock_cache_instance

        main()

        mock_worker_class.assert_called_once_with(mock_db, mock_llm, cache_store=mock_cache_instance)
    ...
    @patch('workers.MongoClient')
    @patch('workers.os.getenv')
    def test_runs_worker(self, mock_getenv, mock_mongo_client, mock_llamacpp,
                         mock_submissions_storage, mock_semantic_storage, mock_worker_class):
        """Runs Worker."""
        mock_getenv.side_effect = lambda key, default=None: default
        mock_client_instance = MagicMock()
    ...
    @patch('workers.MongoClient')
    @patch('workers.os.getenv')
    def test_handles_keyboard_interrupt_gracefully(self, mock_getenv, mock_mongo_client, mock_llamacpp,
                                                   mock_submissions_storage, mock_semantic_storage, mock_worker_class,
                                                   caplog):
        ...
        mock_mongo_client.return_value = mock_client_instance
        mock_worker_instance = MagicMock()
        mock_worker_instance.run.side_effect = KeyboardInterrupt()
        mock_worker_class.return_value = mock_worker_instance
    ...
    @patch('workers.MongoClient')
    @patch('workers.os.getenv')
    def test_closes_mongodb_connection_in_finally_block(self, mock_getenv, mock_mongo_client, mock_llamacpp,
                                                        mock_submissions_storage, mock_semantic_storage, mock_worker_class):
        """Closes MongoDB connection in finally block."""
        mock_getenv.side_effect = lambda key, default=None: default
        mock_client_instance = MagicMock()
        mock_mongo_client.return_value = mock_client_instance
        mock_worker_instance = MagicMock()
    ...
    @patch('workers.MongoClient')
    @patch('workers.os.getenv')
    def test_removes_completed_tasks_older_than_2_days_on_startup(
            self, mock_getenv, mock_mongo_client, mock_llamacpp,
            mock_submissions_storage, mock_semantic_storage, mock_cache_store, mock_worker_class):
        """On startup, deletes completed tasks with completed_at older than 2 days."""
        mock_getenv.side_effect = lambda key, default=None: default
        mock_client_instance = MagicMock()
        mock_mongo_client.return_value = mock_client_instance
        mock_db = MagicMock()
        ...
        call_filter = mock_db.task_queue.delete_many.call_args[0][0]
        assert call_filter["status"] == "completed"
        assert "$lt" in call_filter["completed_at"]
        # The cutoff must be approximately 2 days in the past
        cutoff = call_filter["completed_at"]["$lt"]
        age = datetime.now(UTC) - cutoff
        assert timedelta(days=1, hours=23) < age < timedelta(days=2, hours=1)


class TestEdgeCases:
    """Test edge cases for the worker."""

    @patch('workers.signal.signal')
    @patch('workers.SubmissionsStorage')
    @patch('workers.SemanticDiffsStorage')
    def test_worker_shutdown_during_task_processing(self, mock_semantic_storage, mock_submissions_storage, mock_signal):
        ...
        mock_handler = MagicMock()
        mock_handler.side_effect = lambda *args, **kwargs: setattr(worker, 'running', False)
    ...
    @patch('workers.SubmissionsStorage')
    @patch('workers.SemanticDiffsStorage')
    def test_database_connection_loss(self, mock_semantic_storage, mock_submissions_storage, mock_signal):
        """Database connection loss."""
        mock_db = MagicMock()
        mock_llm = MagicMock()
        worker = Worker(mock_db, mock_llm)

        # Simulate database connection loss on first call
        call_count = [0]
    ...
    @patch('workers.SubmissionsStorage')
    @patch('workers.SemanticDiffsStorage')
    def test_llm_unavailable(self, mock_semantic_storage, mock_submissions_storage, mock_signal):
        """LLM unavailable."""
        mock_db = MagicMock()
        mock_llm = MagicMock()
        worker = Worker(mock_db, mock_llm)
        ...
        mock_handler = MagicMock(side_effect=Exception("LLM service unavailable"))
        ...
        with patch('workers.TASK_HANDLERS') as mock_handlers:
            ...
            with patch.object(worker, '_mark_task_failed') as mock_mark_failed:
                with patch.object(worker, '_mark_task_completed'):
                    worker.process_task(mock_task)

                    mock_mark_failed.assert_called_once()
                    assert "LLM service unavailable" in mock_mark_failed.call_args[0][1]

    @patch('workers.signal.signal')
    @patch('workers.SubmissionsStorage')
    @patch('workers.SemanticDiffsStorage')
    def test_task_handler_timeout(self, mock_semantic_storage, mock_submissions_storage, mock_signal):
        """Task handler hangs/times out."""
        mock_db = MagicMock()
        mock_llm = MagicMock()
        worker = Worker(mock_db, mock_llm)
        ...
        # Simulate handler that times out
        mock_handler = MagicMock(side_effect=TimeoutError("Handler timeout"))
    ...
    @patch('workers.SubmissionsStorage')
    @patch('workers.SemanticDiffsStorage')
    def test_rapid_task_submission_vs_processing_rate(self, mock_semantic_storage, mock_submissions_storage, mock_signal):
        """Rapid task submission vs processing rate."""
        mock_db = MagicMock()
        mock_llm = MagicMock()
        worker = Worker(mock_db, mock_llm)
        ...
        tasks = [
            {"_id": f"task-{i}", "task_type": "split_topic_generation", "submission_id": f"sub-{i}"}
            for i in range(10)
        ]
        ...
        def mock_claim_task():
            call_count[0] += 1
            if call_count[0] <= len(tasks):
                return tasks[call_count[0] - 1]
            worker.running = False
            return None
        ...
        assert call_count[0] == len(tasks) + 1  # +1 for the final None return
...
class TestIntegrationPoints:
    ...
    @patch('workers.SubmissionsStorage')
    @patch('workers.SemanticDiffsStorage')
    def test_task_queue_collection_interaction(self, mock_semantic_storage, mock_submissions_storage, mock_signal):
        """MongoDB task_queue collection interaction."""
        mock_db = MagicMock()
        mock_llm = MagicMock()
        worker = Worker(mock_db, mock_llm)

        mock_task = {"_id": "task-1", "task_type": "split_topic_generation", "submission_id": "sub-1"}
        mock_db.task_queue.find_one_and_update.return_value = mock_task

        worker.claim_task()

        # Verify task_queue was accessed with correct parameters
        mock_db.task_queue.find_one_and_update.assert_called_once()
        call_args = mock_db.task_queue.find_one_and_update.call_args
        # Verify the filter includes status: pending
        assert call_args[0][0]["status"] == "pending"
        # Verify the update sets status to processing
        assert "$set" in call_args[0][1]
        assert call_args[0][1]["$set"]["status"] == "processing"
        # Verify sort is provided
        assert "sort" in call_args[1]

    @patch('workers.signal.signal')
    @patch('workers.SubmissionsStorage')
    @patch('workers.SemanticDiffsStorage')
    def test_submissions_collection_interaction(self, mock_semantic_storage, mock_submissions_storage, mock_signal):
        """MongoDB submissions collection interaction."""
        mock_db = MagicMock()
        mock_llm = MagicMock()
        worker = Worker(mock_db, mock_llm)
        ...
        with patch.object(worker, '_mark_task_completed'):
            with patch('workers.process_split_topic_generation'):
                worker.process_task(task)

        # Verify submissions was accessed with correct submission_id
        worker.submissions_storage.get_by_id.assert_called_once_with("sub-123")

    @patch('workers.signal.signal')
    @patch('workers.SubmissionsStorage')
    @patch('workers.SemanticDiffsStorage')
    def test_semantic_diff_jobs_collection_interaction(self, mock_semantic_storage, mock_submissions_storage, mock_signal):
        """MongoDB semantic_diff_jobs collection interaction."""
        mock_db = MagicMock()
        mock_llm = MagicMock()
        worker = Worker(mock_db, mock_llm)
        ...
        worker.claim_diff_job()

        # Verify semantic_diff_jobs was accessed with correct worker_id
        worker.semantic_diffs_storage.claim_job.assert_called_once_with(worker.worker_id)

    @patch('workers.signal.signal')
    @patch('workers.SubmissionsStorage')
    @patch('workers.SemanticDiffsStorage')
    def test_semantic_diffs_collection_interaction(self, mock_semantic_storage, mock_submissions_storage, mock_signal):
        """MongoDB semantic_diffs collection interaction."""
        mock_db = MagicMock()
        mock_llm = MagicMock()
        worker = Worker(mock_db, mock_llm)
            ...
            with patch('workers.compute_topic_aware_semantic_diff'):
                with patch.object(worker.semantic_diffs_storage, 'upsert_diff') as mock_upsert:
                    with patch.object(worker, '_mark_diff_job_completed'):
                        ...
                        # Verify semantic_diffs upsert_diff was called with correct arguments
                        mock_upsert.assert_called_once()
                        call_kwargs = mock_upsert.call_args[1]
                        assert call_kwargs["pair_key"] == "pair-1"
                        assert call_kwargs["submission_a_id"] == "sub-a"
                        assert call_kwargs["submission_b_id"] == "sub-b"
