...
def mock_split_article_with_markers():
    ...
    with patch(
        "lib.tasks.split_topic_generation.split_article_with_markers"
    ) as mock_split:
        yield mock_split
...
class TestProcessSplitTopicGenerationBasic:
    ...
    def test_keeps_full_html_as_splitter_source(
        self,
        mock_db,
        mock_llm,
        mock_split_article_with_markers,
        ...
        """The splitter receives the original HTML including tables."""
        submission = {
            "submission_id": "test-submission-123",
        ...
        split_result.topics = [{"name": "Topic A", "sentences": [1]}]
        ...
        split_call = mock_split_article_with_markers.call_args
        assert split_call[0][0] == submission["html_content"]

        update_call = mock_storage_instance.update_results.call_args
        assert update_call[0][1]["topics"] == split_result.topics
        assert "table_blocks" not in update_call[0][1]
...
class TestProcessSplitTopicGenerationTracer:
    ...
    def test_logs_failure_diagnostics_from_tracer(
        ...
        with (
            patch("lib.tasks.split_topic_generation.Tracer") as mock_tracer_class,
            patch("lib.tasks.split_topic_generation.time.sleep"),
        ):
...
class TestProcessSplitTopicGenerationEdgeCases:
    ...
    def test_handles_missing_results_key(
        ...
        submission = {"submission_id": "test-123", "html_content": "Sample content"}
        mock_split_article_with_markers.return_value = mock_split_result
        mock_storage_instance = MagicMock()
        mock_submissions_storage.return_value = mock_storage_instance
    ...
    def test_handles_missing_submission_id(
        ...
        submission = {"html_content": "Sample content", "results": {}}
        mock_split_article_with_markers.return_value = mock_split_result
        mock_storage_instance = MagicMock()
        mock_submissions_storage.return_value = mock_storage_instance
...
class TestProcessSplitTopicGenerationMocking:
    ...
    def test_mocks_mongodb_database(
        ...
        with patch(
            "lib.tasks.split_topic_generation.SubmissionsStorage"
    ...
    def test_mocks_submissions_storage(
        self, mock_db, mock_llm, mock_split_article_with_markers, mock_split_result
    ):
    ...
    def test_mocks_txt_splitt_tracer(
        self, mock_db, mock_llm, mock_split_article_with_markers, mock_split_result
    ):
...
class TestProcessSplitTopicGenerationLLMUnavailable:
    ...
    def test_handles_llm_exception(
        ...
        mock_split_article_with_markers.side_effect = Exception(
