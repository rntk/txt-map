    ...
    (topic, direction = "next") => {
      if (groupedByTopics) {
        const isSelected = (name) =>
          selectedTopics.some((t) => t.name === name);
        ...
        if (direction === "focus") {
          ...
          const allSections = Array.from(
            document.querySelectorAll('[id^="grouped-topic-"]'),
          );
          ...
          if (direction === "next") {
            targetIdx =
              currentIdx < allSections.length - 1 ? currentIdx + 1 : currentIdx;
          } else {
