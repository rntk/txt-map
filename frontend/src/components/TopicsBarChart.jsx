...
function TopicsBarChart({
  ...
  const chartData = useMemo(() => {
    const scopedData = buildScopedChartData(
      topics,
      sentences,
      scopePath,
      selectedLevel,
    ...
    return scopedData
      ...
      .sort(
        (a, b) =>
          a.totalChars - b.totalChars || a.fullPath.localeCompare(b.fullPath),
      );
    ...
    <div className="topics-bar-chart chart-surface chart-surface--topics">
      ...
      <div className="topics-bar-chart__controls">
        <Breadcrumbs
          ...
          classPrefix="topics-bar-chart__"
        />
      ...
      {chartData.length === 0 ? (
        ...
        <>
          <div
            className="topics-bar-chart__body"
            data-testid="topics-bar-chart-scroll"
          >
            {chartData.map((item, index) => {
              const scaledBarWidthPercent =
                (item.totalChars / maxChars) * MAX_BAR_WIDTH_PERCENT;
          ...
          <div className="topics-bar-chart__legend chart-legend">
            {chartData.map((item) => (
              <div
                key={item.fullPath}
                className="topics-bar-chart__legend-item chart-legend-item"
              >
                <div
                ...
                />
                <span className="topics-bar-chart__legend-label">
                  {item.displayName}
                </span>
              </div>
            ))}
