...
describe("TopicSentencesModal markup resolution", () => {
  ...
  it("resolves markup by fullPath when the topic only has a shortened displayName", () => {
    ...
    expect(
      screen.getAllByRole("button", { name: "Scroll to sentence 1" }).length,
    ).toBeGreaterThan(0);
    expect(
      ...
      screen.getAllByText((content, element) =>
        element
          ...
          ?.textContent?.includes(
            "Quantum mechanics changed physics. Researchers debated the implications.",
          ),
      ).length,
    ).toBeGreaterThan(0);
    expect(
      screen.getByText(
        "A later discovery shifted the field. The community adopted the new model.",
      ),
    ).toBeInTheDocument();
  });
  ...
  it("scrolls to an already rendered sentence when the modal minimap is clicked", async () => {
    ...
    expect(
      container.querySelector(".grid-view-minimap-bar--active"),
    ).toBeInTheDocument();
  });
  ...
  it("switches to sentences and reveals context when a minimap click targets a hidden sentence", async () => {
    ...
    fireEvent.click(
      screen.getAllByRole("button", { name: "Scroll to sentence 7" })[0],
    );
