...
describe("StatusIndicator", () => {
  ...
  it("renders task pills with state classes and opens a detail popover", () => {
    ...
    const processingButton = screen.getByRole("button", {
      name: /build queue processing/i,
    });
    expect(processingButton).toHaveClass("shared-status-pill--processing");
    expect(within(processingButton).getByText("⟳")).toHaveClass(
      "shared-status-pill__icon--processing",
    );
    ...
    expect(within(detail).getByText("processing")).toHaveClass(
      "shared-status-detail__status--processing",
    );
  ...
  it("shows task errors and closes on outside click", async () => {
    ...
    expect(screen.getByText("Queue write failed")).toHaveClass(
      "shared-status-detail__error",
    );
