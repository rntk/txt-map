...
function renderHeaderLabel(g, x, y, width, fontSize, textColor, label) {
  const maxChars = Math.max(
    1,
    Math.floor(
      (width - TREEMAP_LABEL_PADDING_X * 2) / (fontSize * CHAR_ASPECT),
    ),
  );
  const displayLabel =
...
export default function TreemapChart({
  ...
  useEffect(() => {
    ...
    const height = Math.min(
      TREEMAP_MAX_HEIGHT,
      Math.max(360, Math.round(containerWidth * 0.8)),
    );
    ...
    d3
      ...
      .paddingTop((node) =>
        node.depth > 0 && node.children ? TREEMAP_HEADER_HEIGHT : 0,
      )(root);
    ...
    nodes.forEach((node) => {
      ...
      const fillStrokeColor =
        d3
          .color(fillColor)
          ?.darker(node.depth === 1 ? 0.35 : 0.22)
          ?.toString() || "#666";
      ...
      const group = g
        ...
        .attr(
          "class",
          isLeaf
            ? "treemap-node treemap-node--leaf"
            : "treemap-node treemap-node--group",
        );
      ...
      const rect = group
        ...
        .attr(
          "class",
          `treemap-node__rect${isInteractive ? " treemap-node__rect--interactive" : ""}`,
        );
        ...
        .on("click", (event) => {
          ...
          if (node.data.topic) {
            setModalTopic({
              ...
              sentenceIndices: Array.isArray(node.data.topic.sentences)
                ? node.data.topic.sentences
                : [],
              ranges: Array.isArray(node.data.topic.ranges)
                ? node.data.topic.ranges
                : [],
            });
          }
        });
      ...
      if (isLeaf) {
        ...
        const fontSize = Math.min(
          16,
          Math.max(9, Math.min(rectWidth * 0.12, rectHeight * 0.28)),
        );
        const maxLines = Math.max(
          1,
          Math.floor((rectHeight - 8) / (fontSize * 1.2)),
        );
        const lines = wrapLines(node.data.name, rectWidth - 10, fontSize).slice(
          0,
          maxLines,
        );
  ...
  if (!topics || topics.length === 0) {
    return (
      <p className="treemap-no-data treemap-no-data--standalone">
        No topics available.
      </p>
    );
  }
  ...
  return (
    <div
      ref={containerRef}
      className="treemap-chart chart-surface chart-surface--treemap"
    >
      <Breadcrumbs
        scopePath={scopePath}
        onNavigate={(path) => {
      ...
      <p className="treemap-subtitle chart-section__copy">{subtitle}</p>
