 ...
 */
export default function DataLineChart({ extraction, width = 340, height = 180 }) {
  ...
  useEffect(() => {
    ...
    g.append('g')
      .attr('class', 'grid')
      .call(
        d3.axisLeft(yScale).ticks(4).tickSize(-innerWidth).tickFormat('')
      )
    ...
    const yAxisG = g.append('g').call(
      d3.axisLeft(yScale).ticks(4).tickFormat((v) => {
