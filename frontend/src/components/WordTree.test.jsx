...
describe("WordTree helpers", () => {
  it("strips HTML before matching tree text", () => {
    expect(sentenceToPlainText("<p>Hello <strong>beta</strong></p>")).toBe(
      "Hello beta",
    );
  });
  ...
  it("creates one entry per occurrence and marks read sentences", () => {
    ...
    expect(entries.filter((entry) => entry.sentenceNumber === 1)).toHaveLength(
      2,
    );
    expect(
      entries
        .filter((entry) => entry.sentenceNumber === 2)
        .every((entry) => entry.isRead),
    ).toBe(true);
  });
