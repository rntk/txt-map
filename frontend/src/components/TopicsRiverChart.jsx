...
const TopicsRiverChart = ({
  ...
  const scopedData = useMemo(() => {
    ...
    return data.map((d) => ({
      ...
      avgCharsPerSentence:
        d.totalChars && d.sentenceIndices.length
          ? d.totalChars / d.sentenceIndices.length
          : 100,
    }));
  ...
  useEffect(() => {
    ...
    let data = calculateBins(binCount, scopedData, {
      start: 0,
      end: effectiveLength,
    });
    ...
    scopedData.forEach((topic) => {
      totalSentencesPerTopic[topic.name] = topic.sentences
    ...
    g.selectAll(".stream-layer")
      ...
      .on("mouseover", function (event, d) {
        ...
        tooltip
          .style("opacity", 1)
          .html(
            `<strong>${d.key}</strong><br/>Total: ${totalSentences} sentences`,
          )
  ...
  return (
    <div
      ref={containerRef}
      className="topics-river-chart chart-surface chart-surface--river"
    >
      <TopicLevelSwitcher
