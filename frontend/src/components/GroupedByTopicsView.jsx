...
export default function GroupedByTopicsView({ topics, rawHtml, sentences, isRawTextMode, highlightedTopicName }) {
  const sortedTopics = useMemo(() => {
    ...
    return [...topics].sort((a, b) => {
      ...
      const aMin = aRanges.length > 0
        ? Math.min(...aRanges.map(r => Number(r.start)))
        : (Array.isArray(a.sentences) && a.sentences.length > 0 ? Math.min(...a.sentences) : Infinity);

      const bMin = bRanges.length > 0
        ? Math.min(...bRanges.map(r => Number(r.start)))
        : (Array.isArray(b.sentences) && b.sentences.length > 0 ? Math.min(...b.sentences) : Infinity);
  ...
  return (
    <div className="grouped-topics-list">
      {sortedTopics.map(topic => {
        ...
        } else if (Array.isArray(topic.sentences) && topic.sentences.length > 0 && Array.isArray(sentences)) {
        ...
        return (
          ...
          >
            ...
            {isRawTextMode ? (
              ...
              <div className="grouped-topic-text">{extractedText || '(no text)'}</div>
            )}
          </div>
        );
      })}
