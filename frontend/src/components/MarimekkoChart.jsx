...
function computeRowHeights(rows, colHeight) {
  ...
  const values = rows.map((row) =>
    Math.sqrt(Math.max(0, Number(row.totalChars || row.value) || 0)),
  );
  ...
  return values.map(
    (value) => MIN_ROW_HEIGHT + (value / valueSum) * extraHeight,
  );
}

function getColWeight(col) {
  const raw = Math.max(
    col.totalChars > 0 ? col.totalChars : col.sentenceCount || 1,
    1,
  );
...
function MarimekkoChart({ columns, containerWidth, onBarClick }) {
  const layout = useMemo(() => {
    ...
    const baseBarAreaHeight = Math.max(
      Math.round(
        (typeof window !== "undefined" ? window.innerHeight : 600) * 0.65,
      ),
      400,
    );
    ...
    const availableWidth = Math.max(
      ...
      columns.length * MIN_COL_WIDTH,
    );

    // Natural column widths
    const colWidths = columns.map((col) =>
      Math.max(
        (getColWeight(col) / totalWeight) * availableWidth,
        MIN_COL_WIDTH,
      ),
    );
    ...
    const colHeights = columns.map((col) => {
      ...
      const minReadable = Math.max(
        MIN_COL_HEIGHT,
        col.rows.length * MIN_ROW_HEIGHT,
      );
  ...
  columns.forEach((col, colIdx) => {
    ...
    col.rows.forEach((row, rowIdx) => {
      ...
      const isClickable = !!(
        onBarClick &&
        (row.isDrillable ||
          (row.sentenceIndices && row.sentenceIndices.length > 0))
      );
      ...
      elements.push(
        <rect
          ...
          className={
            isClickable
              ? "marimekko-chart__bar marimekko-chart__bar--interactive"
              : "marimekko-chart__bar"
          }
      ...
      if (rowHeight >= MIN_ROW_HEIGHT && colWidth >= 50) {
        ...
        const labelText =
          row.displayName.length > maxChars
    ...
    const colLabel =
      col.displayName.length > maxLabelLen
  ...
  return (
    <svg
      width={svgWidth}
      height={svgHeight}
      className="marimekko-chart__svg chart-svg"
    >
      {elements}
    </svg>
  );
