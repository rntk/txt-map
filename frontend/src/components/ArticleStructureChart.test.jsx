...
import ArticleStructureChart, { buildScopedChartData, getScopedMaxLevel } from './ArticleStructureChart';
...
describe('ArticleStructureChart scope helpers', () => {
  ...
  it('builds chart data only for descendants within the current scope', () => {
    ...
    expect(rootData.map(item => item.displayName)).toEqual(['Science', 'Arts']);

    const scienceLevelZero = buildScopedChartData(topics, sentences, ['Science'], 0);
    expect(scienceLevelZero.map(item => item.displayName)).toEqual(['Physics', 'Biology']);

    const scienceLevelOne = buildScopedChartData(topics, sentences, ['Science'], 1);
    expect(scienceLevelOne.map(item => item.displayName)).toEqual(['Quantum']);
...
describe('ArticleStructureChart component', () => {
  ...
  it('drills into a branch from a deeper level and resets to level 0', () => {
    ...
    fireEvent.click(screen.getByTestId('article-structure-block-science-physics'));
    ...
    expect(screen.queryByTestId('article-structure-block-arts-music')).not.toBeInTheDocument();
  });
  ...
  it('does not drill into a leaf topic', () => {
    ...
    fireEvent.click(screen.getByTestId('article-structure-block-science'));
    fireEvent.click(screen.getByTestId('article-structure-block-science-biology'));
