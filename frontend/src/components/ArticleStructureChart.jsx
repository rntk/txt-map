...
function ArticleStructureChart({
  ...
  const layout = useMemo(() => {
    ...
    const widths = chartData.map((item) =>
      Math.max(
        (item.sentenceCount / totalSentences) * availableWidth,
        MIN_BLOCK_WIDTH,
      ),
    );
  ...
  const { linePath, areaPath, yTicks, xTicks } = useMemo(() => {
    const maxVal =
      smoothedCounts.length > 0 ? Math.max(...smoothedCounts, 1) : 1;
    const n = smoothedCounts.length;

    const xS = (i) =>
      MARGIN.left + (n > 1 ? i / (n - 1) : 0.5) * layout.plotWidth;
    ...
    const xTicksArr = Array.from({ length: xTickCount }, (_, i) => {
      ...
      return {
        ...
        x: MARGIN.left + frac * layout.plotWidth,
      };
    });
  ...
  return (
    <div
      ref={containerRef}
      className="article-structure-chart chart-surface chart-surface--article-structure"
    >
      <div className="article-structure-controls chart-surface__controls">
        ...
        <p className="article-structure-scope-copy chart-section__copy">
          {subtitle}
        </p>
      </div>
        ...
        <div className="article-structure-scroll-container">
          ...
          >
            ...
            {layout.blocks.map((block) => {
              ...
              const label =
                block.displayName.length > maxChars
        ...
        <div className="article-structure-legend chart-legend">
          {chartData.map((item) => (
            ...
            >
              ...
              />
              <span className="article-structure-legend-name">
                {item.displayName}
              </span>
              <span className="article-structure-legend-value">
      ...
      {tooltip && (
        ...
        >
          <div className="article-structure-tooltip-name">
