...
describe("summaryTimeline utilities", () => {
  describe("splitTopicPath", () => {
    it("splits and trims topic paths", () => {
      expect(splitTopicPath("Design > Structures > Loads")).toEqual([
        "Design",
        "Structures",
        "Loads",
      ]);
  ...
  describe("chooseSummaryTopic", () => {
    it("prefers the deepest overlapping topic path", () => {
      ...
      expect(match.pathSegments).toEqual([
        "Design",
        "Structures",
        "Load Paths",
      ]);
  ...
  describe("buildSummaryTimelineItems", () => {
    it("marks section labels only when the top-level topic changes", () => {
      ...
      expect(items.map((item) => item.showSectionLabel)).toEqual([
        true,
        false,
        true,
      ]);
      expect(items.map((item) => item.topLevelLabel)).toEqual([
        "Sophomore year",
        "Sophomore year",
        "Junior year",
      ]);
      expect(items.map((item) => item.subtopicLabel)).toEqual([
        "Revit",
        "Structures",
        "Competition",
      ]);
