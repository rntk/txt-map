import { normalizeCharRange, buildTopicStateRanges, buildRawTextSegments } from './textHighlight';

describe('normalizeCharRange', () => {
  test('returns clamped range for valid input', () => {
    expect(normalizeCharRange({ start: 2, end: 8 }, 100)).toEqual({ start: 2, end: 8 });
  });

  test('clamps start below 0', () => {
    expect(normalizeCharRange({ start: -5, end: 8 }, 100)).toEqual({ start: 0, end: 8 });
  });

  test('clamps end above textLength', () => {
    expect(normalizeCharRange({ start: 90, end: 150 }, 100)).toEqual({ start: 90, end: 100 });
  });
...
describe('buildTopicStateRanges', () => {
  ...
  test('selected topic goes into highlightRanges', () => {
    const { highlightRanges, fadeRanges } = buildTopicStateRanges(
      topics, [{ name: 'A' }], null, new Set(), 100
    );
  ...
  test('selected takes priority over read', () => {
    const { highlightRanges, fadeRanges } = buildTopicStateRanges(
      topics, [{ name: 'A' }], null, new Set(['A']), 100
    );
  ...
  test('topics with no ranges are skipped', () => {
    ...
    const { highlightRanges, fadeRanges } = buildTopicStateRanges(
      topicsNoRanges, [{ name: 'X' }], null, new Set(), 100
    );
    expect(highlightRanges).toHaveLength(0);
    expect(fadeRanges).toHaveLength(0);
  });
  ...
  test('handles array readTopics (non-Set)', () => {
    const { fadeRanges } = buildTopicStateRanges(
      topics, [], null, ['C'], 100
    );
    expect(fadeRanges).toEqual([{ start: 30, end: 40 }]);
  });
...
describe('buildRawTextSegments', () => {
  ...
  test('splits text into highlighted and plain segments', () => {
    const segments = buildRawTextSegments('hello world', [{ start: 0, end: 5 }], []);
  ...
  test('faded segment', () => {
    const segments = buildRawTextSegments('hello world', [], [{ start: 6, end: 11 }]);
    expect(segments).toHaveLength(2);
  ...
  test('merges adjacent segments of same state', () => {
    ...
    const segments = buildRawTextSegments('abcde', [{ start: 0, end: 2 }, { start: 2, end: 5 }], []);
    expect(segments).toHaveLength(1);
