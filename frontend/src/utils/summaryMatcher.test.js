...
describe('bagOfWordsScore', () => {
  ...
  it('returns 1 when all query tokens appear in candidate', () => {
    expect(bagOfWordsScore('machine learning models', 'deep machine learning models are popular')).toBe(1);
  });

  it('returns partial score when some query tokens match', () => {
    const score = bagOfWordsScore('machine learning algorithms', 'machine learning is great');
...
describe('matchSummaryToTopics', () => {
  ...
  it('accepts a custom matcherFn', () => {
    ...
    const results = matchSummaryToTopics('anything', topics, sentences, alwaysMatch);
  ...
  it('does not include topics below the score threshold', () => {
    ...
    const neverMatch = () => 0;
    const results = matchSummaryToTopics('machine learning', topics, sentences, neverMatch);
    expect(results).toEqual([]);
  });
});
