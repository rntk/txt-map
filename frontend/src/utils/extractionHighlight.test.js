...
describe("getExtractionValues", () => {
  test("returns trimmed non-empty values only", () => {
    expect(
      getExtractionValues({
        values: [{ value: " 48% " }, { value: "" }, { value: "Europe" }],
      }),
    ).toEqual(["48%", "Europe"]);
  });
});
...
describe("extractionIncludesSentence", () => {
  test("checks source sentence membership", () => {
    expect(extractionIncludesSentence({ source_sentences: [2, 4] }, 2)).toBe(
      true,
    );
    expect(extractionIncludesSentence({ source_sentences: [2, 4] }, 3)).toBe(
      false,
    );
  });
});
...
describe("buildExtractionTextSegments", () => {
  test("matches values case-insensitively", () => {
    const segments = buildExtractionTextSegments(
  ...
  test("returns a single plain segment when nothing matches", () => {
    expect(
      buildExtractionTextSegments("Revenue grew by 48%.", {
        values: [{ value: "49%" }],
      }),
    ).toEqual([
