...
describe('buildDiffRows', () => {
  ...
  it('each row has an id field with the format row-N', () => {
    const diffState = {
      diff: {
        unmatched_left: [
          { left_sentence_index: 0, left_text: 'A', right_sentence_index: null, similarity: null },
          { left_sentence_index: 1, left_text: 'B', right_sentence_index: null, similarity: null },
        ],
      },
    };
  ...
  it('rows are sorted by left sentence index then right sentence index', () => {
    const diffState = {
      diff: {
        matches_left_to_right: [
          { left_sentence_index: 2, left_text: 'C', right_sentence_index: 0, right_text: 'c', similarity: 0.7 },
          { left_sentence_index: 0, left_text: 'A', right_sentence_index: 1, right_text: 'a', similarity: 0.9 },
        ],
      },
    };
  ...
  it('nearestRight and nearestLeft exclude the paired match', () => {
    ...
    const diffState = {
      diff: {
        matches_left_to_right: [
          { left_sentence_index: 0, left_text: 'L0', right_sentence_index: 0, right_text: 'R0', similarity: 0.9 },
          { left_sentence_index: 0, left_text: 'L0', right_sentence_index: 1, right_text: 'R1', similarity: 0.5 },
        ],
        unmatched_right: [
          { left_sentence_index: null, right_sentence_index: 1, right_text: 'R1', similarity: null },
        ],
      },
    };
    ...
    expect(matchRow.nearestRight.every(r => r.right_sentence_index !== 0)).toBe(true);
  });
  ...
  it('handles edges with zero similarity — they are not added as edges', () => {
    ...
    const diffState = {
      diff: {
        matches_left_to_right: [
          { left_sentence_index: 0, left_text: 'A', right_sentence_index: 0, right_text: 'B', similarity: 0 },
        ],
      },
    };
