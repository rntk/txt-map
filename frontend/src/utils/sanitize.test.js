...
describe('sanitizeHTML', () => {
  ...
  describe('background-color', () => {
    it('always strips background-color', () => {
      const style = getStyle('<div style="background-color: rgb(255, 0, 0)">text</div>');
      expect(style).toBeNull();
    });

    it('strips even transparent background-color', () => {
      const style = getStyle('<div style="background-color: rgba(0, 0, 0, 0)">text</div>');
      expect(style).toBeNull();
    });
  });
  ...
  describe('layout properties stripped entirely', () => {
    ...
    it('strips border-top', () => {
      const style = getStyle('<div style="border-top: 1px solid black">text</div>');
      expect(style).toBeNull();
    });
  });
  ...
  describe('font-family generic extraction', () => {
    it('extracts generic family from stack', () => {
      const style = getStyle('<p style="font-family: &quot;Helvetica Neue&quot;, Arial, sans-serif">text</p>');
      expect(style).toBe('font-family: sans-serif');
    });

    it('extracts monospace from stack', () => {
      const style = getStyle('<pre style="font-family: Courier, monospace">text</pre>');
      expect(style).toBe('font-family: monospace');
    });

    it('strips font-family with no generic family', () => {
      const style = getStyle('<p style="font-family: Arial, Helvetica">text</p>');
      expect(style).toBeNull();
    });
  });
  ...
  describe('combined style string', () => {
    it('keeps safe props, strips dangerous ones, clamps out-of-range ones', () => {
      const input = '<p style="font-weight: bold; width: 800px; font-size: 72px; background-color: rgb(0,0,0); color: rgb(50,50,50); display: flex; margin-top: 10px">text</p>';
  ...
  describe('safe formatting properties pass through', () => {
    ...
    it('keeps text-decoration underline', () => {
      const style = getStyle('<span style="text-decoration: underline">text</span>');
